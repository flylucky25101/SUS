...
const PLAYER_SPRITE_URL = './assets/player-spritesheet.png';
const PLAYER_SPRITE_FRAME_SIZE = 64;
const PLAYER_SPRITE_DISPLAY_SIZE = 128;
const PLAYER_SPRITE_FOOT_OFFSET_Y = 8;
...
export class ArenaScene extends Phaser.Scene {
  ...
  private playerSpriteAnimator: SpriteAnimator | null = null;
  private lastPlayerAnimation: SpriteAnimationName = 'idle';
  private lastPlayerStatus: FighterState['status'] = 'idle';
  private lastPlayerAttackFrame = -1;
  private lastPlayerAttackMoveId: string | null = null;
  private lastPlayerHitstunFrames = 0;
  ...
  create(): void {
    ...
    this.playerSpriteAnimator = new SpriteAnimator(
      this,
      PLAYER_SPRITE_URL,
    ...
    this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
      ...
      this.playerSpriteAnimator?.destroy();
      this.playerSpriteAnimator = null;
    });
  ...
  update(_time: number, rawDelta: number): void {
    ...
    this.updatePlayerSprite(rawDelta);
    this.renderWorld();
  ...
  getPlayerSpriteSnapshot(): SpriteAnimatorSnapshot | null {
    return this.playerSpriteAnimator?.snapshot() ?? null;
  }
  ...
  private updatePlayerSprite(rawDelta: number): void {
    const animator = this.playerSpriteAnimator;
    const fighter = this.world.fighters.find((candidate) => candidate.id === 'p1');
    if (animator === null || fighter === undefined) return;
    ...
    const attackRestarted = animation === 'attack' && (
      this.lastPlayerAnimation !== 'attack'
      || (fighter.status === 'attack' && this.lastPlayerStatus !== 'attack')
      || attackMoveId !== this.lastPlayerAttackMoveId
      || (attackFrame >= 0 && attackFrame < this.lastPlayerAttackFrame)
    );
    const hitRestarted = animation === 'hit' && (
      this.lastPlayerAnimation !== 'hit'
      || fighter.hitstunFrames > this.lastPlayerHitstunFrames
    );
    ...
    this.lastPlayerAnimation = animation;
    this.lastPlayerStatus = fighter.status;
    this.lastPlayerAttackFrame = attackFrame;
    this.lastPlayerAttackMoveId = attackMoveId;
    this.lastPlayerHitstunFrames = fighter.hitstunFrames;
  }
  ...
  private drawFighter(fighter: FighterState, graphics: Phaser.GameObjects.Graphics): void {
    ...
    const spriteDrawn = fighter.id === 'p1' && this.playerSpriteAnimator?.draw({
      x,
      y: fighter.position.y + PLAYER_SPRITE_FOOT_OFFSET_Y,
      width: PLAYER_SPRITE_DISPLAY_SIZE,
      height: PLAYER_SPRITE_DISPLAY_SIZE,
      flipX: facing < 0,
      alpha,
      depth: 4,
    }) === true;
