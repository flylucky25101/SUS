...
type Screen = 'loading' | 'onboarding' | 'title' | 'main' | 'fighters' | 'stages' | 'difficulty' | 'settings' | 'help' | 'game';
  ...
  private renderMainMenu(): void {
    ...
    this.root.innerHTML = this.shell(`<section class="main-menu">
      ...
      <nav class="menu-actions" aria-label="${t(language, 'mainMenu')}">
        ...
        <button class="menu-tile" data-action="help"><span class="menu-index">03</span><strong>${t(language, 'howToPlay')}</strong><small>INPUT // RECOVERY</small><i aria-hidden="true"></i></button>
        <button class="menu-tile" data-action="open-settings"><span class="menu-index">04</span><strong>${t(language, 'settings')}</strong><small>ACCESS // AUDIO // UI</small><i aria-hidden="true"></i></button>
      ...
      <aside class="menu-art" aria-hidden="true"><div class="rift-figure"><i></i><b></b><span></span></div><p>NO CROWN.<br>ONLY CONTROL.</p></aside>
    </section>`, 'menu-layout');
  }
  ...
  private renderFighterSelect(): void {
    ...
    const cards = FIGHTERS.map((fighter) => {
      ...
      return `<button class="fighter-card ${selected ? 'is-selected' : ''}" style="--fighter:${colorHex(fighter.color)};--accent:${colorHex(fighter.accent)}" data-action="select-fighter" data-value="${fighter.id}" data-testid="fighter-${fighter.id}" aria-pressed="${selected}">
        ...
        <span class="portrait portrait--${fighter.id}" aria-hidden="true"><i class="portrait-head"></i><i class="portrait-body"></i><i class="portrait-gear"></i></span>
